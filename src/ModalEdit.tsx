...
import { ServerComponent } from "@battis/jsx-api-components";
import { _HTML } from "@battis/monkey-patches";
import { Nullable, Subset } from "@battis/typescript-tricks";
import { Modal, ModalConfig } from "@battis/jsx-ui";
    ...
    this._body.element = (
      ...
      >
        <div class="form-controls">
          {this.formatPropertyControls(target, views, foreignKeyLookup)}
        </div>
        <div class="buttons">
      ...
      if (componentType) {
        return (
          <>
            ...
            {_HTML.asyncSelectControl({
              ...
              value: target[key],
              loader: () =>
                ((componentType.type && componentType.type) || componentType)
                  .list()
                  .then((options) =>
                    options.map((option) => {
                      return {
                        id: option.id,
                        name: (option.inline() as HTMLElement).innerText,
                      };
                    })
                  ),
            })}
        ...
        return (
          <>
            ...
            <input type="text" name={key} value={props[key] || ""} />
          </>
        );
      }
    });
